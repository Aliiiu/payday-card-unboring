...
export function pixelToRem(pixels, innerWidth = window.innerWidth) {
  return (innerWidth / 1512) * pixels
}

export function remToPixels(rem, innerWidth = window.innerWidth) {
  return (rem * 1512) / innerWidth
}

export function snap(value, lowerLimit, upperLimit, fractStart = 0, fractEnd = 6) {
...
export function snapTo(normal, upperLimit, lowerLimit, fractStart = 0, fractEnd = 6) {
  if (!lowerLimit) {
